...
def main_loop():
    ...
    while(1):
        ...
        if 'RETRAIN' in example_sentence.split():
            ai_model_feature = FeatureTextToTextAI(conv_labels_answers(sl), num_epochs=epochs)
            ai_model_feature.train()

            ai_model_main = MainTextToTextAI(conv_prompts_liked(ai_model_feature, tl), num_epochs=epochs)
            ai_model_main.train()
            continue
